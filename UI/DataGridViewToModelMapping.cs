		...
		public static List<Employee> GetEmployeesFromGrid(DataGridView employeeDataGrid)
		{
			...
			foreach (DataGridViewRow row in employeeDataGrid.Rows)
			{
				...
				var employee = new Employee
				{
					...
					DateOfBirth = DateTime.TryParse(row.Cells["DateOfBirth"].Value?.ToString(), out var dob) ? dob : null,
					DateEmployedFrom = DateTime.TryParse(row.Cells["DateEmployedFrom"].Value?.ToString(), out var def) ? def : null,
					DateEmployedTo = DateTime.TryParse(row.Cells["DateEmployedTo"].Value?.ToString(), out var det) ? det : null,
